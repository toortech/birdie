...
const BBB_AUTH = {
  ...
  /**
   * Alias for cloudflareLoginWithCaptcha for compatibility
   */
  cloudflareLogin: async function(username, password, captchaToken = null) {
    return this.cloudflareLoginWithCaptcha(username, password, captchaToken);
  },
  ...
  /**
   * FIXED: Check if user has specific permission (unified function)
  ...
  hasPermission: function(permission) {
    ...
    // Use permissions array if available (preferred method)
    ...
    // Fallback permission check based on role
    ...
    // Member permissions
    if (role === 'member' && [
      'view_member_content', 'upload_photos', 'view_own_scorecards',
      'gallery.view_all', 'gallery.upload_limited', 'gallery.delete_own'
    ].includes(permission)) return true;
    
    // Gallery permissions
    if (role === 'gallery' && [
      'view_gallery', 'upload_photos',
      'gallery.view_all', 'gallery.upload_limited', 'gallery.delete_own'
    ].includes(permission)) return true;
